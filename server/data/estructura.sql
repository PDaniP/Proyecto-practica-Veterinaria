...
DROP TABLE IF EXISTS lotes; -- Se elimina primero por la restricción de clave foránea
...
-- 8. Tabla de Productos e Insumos (Catálogo base)
-- Ya NO contiene stock_actual ni fecha_vencimiento porque pasan a la tabla lotes.
CREATE TABLE productos (
    ...
    stock_minimo INT NOT NULL DEFAULT 5, -- Alerta global para el backend
...
-- 9. Tabla de Lotes (Control físico de Stock y Vencimiento para FEFO)
CREATE TABLE lotes (
    id SERIAL PRIMARY KEY,
    ...
    fecha_vencimiento DATE NOT NULL,  -- Columna crítica para ordenar por FEFO
	activo BOOLEAN DEFAULT TRUE,
    CONSTRAINT fk_producto_lote FOREIGN KEY (id_producto) REFERENCES productos(id) ON DELETE CASCADE ON UPDATE CASCADE
);

-- 10. Inserción de tus 6 Categorías Generales 
...
-- 11. Inserción de Productos de Prueba (Datos fijos de catálogo)
INSERT INTO productos (id_categoria, nombre, descripcion, precio_costo, precio_venta, stock_minimo, venta_al_publico) VALUES 
-- Productos para la TIENDA (venta_al_publico = TRUE)
((SELECT id FROM categorias WHERE nombre = 'Alimentos'), 'Alimento Perro Adulto 15kg', 'Comida premium para perros medianos', 40000.00, 55000.00, 5, TRUE),
((SELECT id FROM categorias WHERE nombre = 'Accesorios y Juguetes'), 'Correa Extensible Recorzada 5m', 'Correa color roja para perros hasta 20kg', 5000.00, 8500.00, 3, TRUE),
((SELECT id FROM categorias WHERE nombre = 'Accesorios y Juguetes'), 'Pelota de Goma Irrompible', 'Juguete mordillo para cachorros', 1200.00, 2500.00, 5, TRUE),
((SELECT id FROM categorias WHERE nombre = 'Higiene y Cuidado Diario'), 'Champú Neutro para Mascotas 500ml', 'Champú apto para el pH de perros y gatos', 1500.00, 3200.00, 4, TRUE),
((SELECT id FROM categorias WHERE nombre = 'Higiene y Cuidado Diario'), 'Limpiador de Oídos Solución Otica', 'Limpiador para prevención de otitis', 2200.00, 4500.00, 2, TRUE),

-- Productos para el VETERINARIO (venta_al_publico = FALSE y precio_venta opcional)
((SELECT id FROM categorias WHERE nombre = 'Vacunas'), 'Vacuna Quíntuple Canina', 'Dosis inmunológica para cachorros', 3500.00, NULL, 10, FALSE),
((SELECT id FROM categorias WHERE nombre = 'Medicamentos y Fármacos'), 'Anestésico Inyectable 50ml', 'Frasco para cirugías programadas', 12000.00, NULL, 2, FALSE),
((SELECT id FROM categorias WHERE nombre = 'Descartables e Insumos Médicos'), 'Jeringas desc. 3ml (Caja x100)', 'Insumo clínico diario para aplicaciones', 4500.00, NULL, 3, FALSE);

-- 12. Inserción de Lotes de Prueba (Acá se asigna el inventario real)
...
((SELECT id FROM productos WHERE nombre = 'Correa Extensible Recorzada 5m' LIMIT 1), 'ACC-001', 15, 15, '2030-01-01'), -- Algo que casi no vence igual lleva lote lejano
...
-- Clínica / FEFO en acción: Dos lotes distintos para la Vacuna Quíntuple
((SELECT id FROM productos WHERE nombre = 'Vacuna Quíntuple Canina' LIMIT 1), 'VAC-26-A', 20, 20, '2026-10-15'), -- Vence primero
((SELECT id FROM productos WHERE nombre = 'Vacuna Quíntuple Canina' LIMIT 1), 'VAC-27-B', 30, 30, '2027-02-28'), -- Vence después
